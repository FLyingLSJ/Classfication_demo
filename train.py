...
def main():
    ...
    trainset = datasets.ImageFolder(root=args.trainroot, transform=transform['val_train'])
    train_loader = data.DataLoader(trainset, batch_size=args.train_batch, shuffle=True, num_workers=args.workers, pin_memory=True)

    valset = datasets.ImageFolder(root=args.valroot, transform=transform['val_test'])
